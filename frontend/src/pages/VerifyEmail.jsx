...
export default function VerifyEmail() {
  ...
  const [verifying, setVerifying] = useState(Boolean(queryToken));
  ...
  useEffect(() => {
    if (!queryToken && !stateSession) {
      navigate("/signup", { replace: true });
    }
  }, [navigate, queryToken, stateSession]);
  ...
  useEffect(() => {
    ...
    axios
      .post(`${api_endpoint}/api/auth/email/verify/`, { token: queryToken })
      .then(() => {
        if (!active) return;
        setVerified(true);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.success("A new verification link was sent to your email.");
    } catch (error) {
      toast.error(
        error.response?.data?.error || "Could not resend link. Try again.",
      );
    } finally {
      setResending(false);
    }
  ...
  const handleSaveEmail = async () => {
    ...
    try {
      ...
      setEditingEmail(false);
      setCooldown(RESEND_COOLDOWN_SECONDS);
      toast.success("Email updated and a new verification link was sent.");
    } catch (error) {
      toast.error(
      ...
      <main className="relative overflow-hidden pt-28 pb-16 sm:pt-32">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-[0.9fr_0.8fr] lg:px-8">
          ...
          <section className="rounded-[2rem] border border-white/70 bg-white p-6 shadow-[0_30px_90px_rgba(33,23,34,0.18)] sm:p-8">
            ...
            {queryToken ? (
              <div className="mt-6 rounded-2xl border border-[#eadfe7] bg-[#fbf8f4] px-4 py-4 text-sm font-bold text-[#665b67]">
                {verifying
                  ? "Please wait while we verify your account."
                  : "If verification did not complete, request a new link from the sign up page."}
              </div>
            ) : (
              <div className="mt-5 flex items-center justify-between gap-3 border-t border-[#eadfe7] pt-5">
                ...
                <button
                  ...
                  disabled={cooldown > 0 || resending || !verificationSession}
