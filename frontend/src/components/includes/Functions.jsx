...
export const giftcardDetailsCalculation = (
  ...
  const localRate = exchangeRate[localCurrency];
  const recipientRate = exchangeRate[recipientCurrency];

  // If recipientCurrency is already USD
  if (recipientCurrency === "USD" && localRate) {
    const amountInLocal = parseFloat(amount) * parseFloat(localRate);
    return amountInLocal.toFixed(2);
  }

  // If both recipient and local currency exist in rates
  if (recipientRate && localRate) {
    const recipientCurrentFirst = 1 / recipientRate;
    const convertedToUSD = parseFloat(amount) * recipientCurrentFirst;
    const convertedToLocal = convertedToUSD * parseFloat(localRate);
    return convertedToLocal.toFixed(2);
  }

  // Fallback if recipient currency not in rates — assume it's in USD
  if (!recipientRate && localRate) {
    const amountInLocal = parseFloat(amount) * parseFloat(localRate);
    return amountInLocal.toFixed(2);
  }

  // Final fallback — can't convert
...
export const TopupExchangeRateConverter = (
